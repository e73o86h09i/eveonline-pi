...
const Header: FC = () => {
  ...
  return (
    <>
      ...
      <Modal open={showMarginInfo} onClose={() => setShowMarginInfo(false)} title="How Margins Are Calculated">
        <div className="space-y-4 text-sm text-gray-300">
          ...
          <p className="text-xs text-gray-400">
            ...
            (lowest sell order) for inputs. This represents the instant-sell profit if you sell output immediately to buy orders and purchase inputs from the
            cheapest sell orders.
          </p>
          <p className="text-xs text-gray-400">
            <span className="font-semibold text-gray-300">Note:</span> Input costs always use market prices, not the cost of producing those inputs yourself.
            For example, a <span className="text-yellow-400">P2</span> margin is based on buying its <span className="text-red-400">P1</span> inputs from the
            market — not on the cost of manufacturing those <span className="text-red-400">P1</span> items from <span className="text-gray-400">R0</span> raw
            resources. Each tier&apos;s margin is calculated independently.
          </p>
        </div>
      </Modal>
    </>
